...
import { Play, Plus, LogOut, Settings, Copy, Users, Clock, ArrowRight, MonitorPlay, CheckCircle2, Loader2 } from "lucide-react";
...
export default function Dashboard() {
  ...
  const handleCreateRoom = async () => {
    if (!newTitle) return;
    const room = await createRoom.mutateAsync({ title: newTitle, ottUrl: newUrl || undefined });
    setShowCreate(false);
    setNewTitle("");
    setNewUrl("");
      ...
      <main className="container mx-auto px-6 py-10 max-w-5xl">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-10">
            ...
            <div className="glass-surface p-6">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { if (joinCode) navigate(`/room/${joinCode}`); }}
          ...
          {showCreate && (
            <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <div className="space-y-3">
                <input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="Room name (e.g., Friday Movie Night)"
                ...
                <input value={newUrl} onChange={(e) => setNewUrl(e.target.value)} placeholder="OTT URL (optional, e.g., netflix.com/title/...)"
                  className="w-full px-4 py-3 rounded-xl bg-secondary border border-white/10 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50" />
          ...
          <div className="mb-10">
            ...
            {roomsLoading ? (
              <div className="glass-surface-subtle p-12 text-center"><Loader2 className="h-8 w-8 animate-spin text-primary mx-auto" /></div>
            ) : !rooms?.length ? (
              <div className="glass-surface-subtle p-12 text-center">
                <MonitorPlay className="h-10 w-10 text-muted-foreground mx-auto mb-3" />
                <p className="text-muted-foreground">No rooms yet. Create one to get started!</p>
              </div>
            ) : (
              <div className="space-y-3">
                {rooms.map((room) => (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 text-xs text-muted-foreground mt-0.5">
                          ...
                          <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {new Date(room.created_at).toLocaleDateString()}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <button onClick={(e) => { e.preventDefault(); navigator.clipboard.writeText(room.slug); toast.success("Code copied!"); }}
                        className="p-2 rounded-lg hover:bg-secondary transition-colors">
                        <Copy className="h-4 w-4 text-muted-foreground" />
                      </button>
                      <span className={`text-xs px-2 py-1 rounded-full ${room.is_active ? "bg-emerald/10 text-emerald" : "bg-secondary text-muted-foreground"}`}>
              ...
              <div className="space-y-3">
                {onboardingSteps.map((step, i) => (
                  <div key={i} className="flex items-center gap-3 text-sm">
                    <CheckCircle2 className={`h-5 w-5 ${step.done ? "text-emerald" : "text-muted-foreground/30"}`} />
            ...
            <div className="glass-surface-subtle p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {supportedPlatforms.map((p) => (
                  <span key={p.name} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${p.color}`}>{p.name}</span>
                ))}
              </div>
              <p className="text-xs text-muted-foreground mt-3">Each participant uses their own subscription.</p>
            </div>
          </div>
        </motion.div>
      </main>
